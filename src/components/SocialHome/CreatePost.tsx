...
function CreatePost():ReactElement {
  ...
  const handleCreatePost = useCallback(async(content:string,file:File)=>{
        try{
            const url = import.meta.env.VITE_BASU_URL_API+'/post/createPost';
  ...
  return (
    <>
        <article className='w-full  p-2 py-3 bg-white shadow-md flex rounded mb-3 gap-6'>
            ...
            }} className='rounded-full flex items-center cursor-pointer transition duration-200 hover:bg-slate-200 p-3 grow border-slate-400 border-2 border-solid'>
                start a post
            </div>
        </article>
