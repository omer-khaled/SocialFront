import {ReactElement,lazy} from 'react'
export const ErrorBoundary = lazy(()=>import('../../utils/ErrorBoundary'));
import { reducerType } from '../../store/store';
import { useSelector } from 'react-redux';
import { authReducerType } from '../../types/type';
import { useNavigate } from 'react-router-dom';
function UserCard():ReactElement{
const {auth,user}:authReducerType = (useSelector<reducerType>(state=>state.auth) as authReducerType);
  const Navigate = useNavigate();
  return (
    <ErrorBoundary error={(!auth)||false}>
        <article className='w-full p-2 bg-white rounded-md'>
            {(user)&&<>
                <img onClick={()=>{
                    Navigate(`/users/${user.id}`);
                }} className='rounded-full cursor-pointer w-[70px] h-[70px] object-cover mx-auto' src={import.meta.env.VITE_BASU_URL_API+'/images/'+user?.image} alt="personalImage" />
                {<h1 className='mt-2 text-center text-lg truncate font-bold'>{user.name}</h1>}
                <hr className='my-2 border-slate-300'/>
                <div className='mt-2 flex items-center justify-between'><div className='flex items-center'><span className='me-1'>number of posts : </span> <span className='text-primary text-lg me-1'>{user.NumberOfPosts}</span></div><svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 16 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M12 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zM4.5 3h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1 0-1zm0 2h7a.5.5 0 0 1 .5.5v8a.5.5 0 0 1-.5.5h-7a.5.5 0 0 1-.5-.5v-8a.5.5 0 0 1 .5-.5z"></path></svg></div>
                <div className='mt-2 flex items-center justify-between'><div className='flex items-center'><span className='me-1'>number of friends : </span> <span className='text-primary text-lg me-1'>{user.numberOfFriends}</span></div><svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 640 512" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M192 256c61.9 0 112-50.1 112-112S253.9 32 192 32 80 82.1 80 144s50.1 112 112 112zm76.8 32h-8.3c-20.8 10-43.9 16-68.5 16s-47.6-6-68.5-16h-8.3C51.6 288 0 339.6 0 403.2V432c0 26.5 21.5 48 48 48h288c26.5 0 48-21.5 48-48v-28.8c0-63.6-51.6-115.2-115.2-115.2zM480 256c53 0 96-43 96-96s-43-96-96-96-96 43-96 96 43 96 96 96zm48 32h-3.8c-13.9 4.8-28.6 8-44.2 8s-30.3-3.2-44.2-8H432c-20.4 0-39.2 5.9-55.7 15.4 24.4 26.3 39.7 61.2 39.7 99.8v38.4c0 2.2-.5 4.3-.6 6.4H592c26.5 0 48-21.5 48-48 0-61.9-50.1-112-112-112z"></path></svg></div>
            </>}
        </article>
    </ErrorBoundary>
  )
}
export default UserCard;
