...
function Navbar():ReactElement {
  ...
  const messages:{sernderId:number}[] = useSelector<reducerType>(state=>state.auth.messages) as {sernderId:number}[];
  ...
  useEffect(()=>{
    if(user){
      ...
      connection.on(`messages/${user.id}`,(data:{action:string,message:messageType})=>{
        if(data.action==='create'){
            dispatch(addMessages(data.message.senderId));
    ...
    <header className='container lg:w-[1000px] xl:w-[1256px] 2xl:w-[1400px] shadow-lg z-[100] p-2 w-full flex justify-between items-center bg-white rounded-md sticky top-0'>
      ...
      <nav>
          <ul className='list-none flex justify-center items-center gap-3'>
            ...
            <NavLink to={'/message'} end className={({isActive})=>{
              ...
              {((messages.length)>0)&&<span className='absolute right-0 top-0 z-[1000] -translate-y-[35%] translate-x-1/2 whitespace-nowrap rounded-full bg-red-500 px-2.5 py-1 text-center text-xs font-bold text-white'>{((messages.length)<50)?(messages.length):"50+"}</span>}
            </NavLink>
            <NavLink to={'/notification'} end className={({isActive})=>{
              const style = "p-1 flex flex-col justify-center items-center hover:text-primary relative";
              return (isActive)?`${style} bg-slate-200 rounded`:`${style}`;
            }}>
